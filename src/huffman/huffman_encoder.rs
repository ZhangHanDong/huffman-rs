...
impl HuffmanEncoder {
    ...
    pub fn encode(&mut self, bytes: Vec<u8>) -> Vec<u8> {
        self.result = vec![];
        ...
        for byte in bytes.iter() {
            let mut code: u32 = u32::from_str_radix(HUFFMAN_CODES[*byte as usize].clone(), 2).unwrap();
            let mut length = HUFFMAN_CODES[*byte as usize].len();

            while length != 0 {
                if space >= length {
                    self.add_code(code << (space - length), space as u32);
                    code = 0;
                    space -= length;
                    length = 0;
                } else {
                    ...
                    let msb = code >> shift;
                    self.add_code(msb, space as u32);
                    code -= msb << shift;
                    length -= space;
                    space = 0;
                }
        ...
        if space != 8 {
            let code: u32 = u32::from_str_radix(HUFFMAN_CODES[256].clone(), 2).unwrap();
            self.add_code(code >> (HUFFMAN_CODES[256].len() - space), space as u32);
        }
    ...
    fn add_code(&mut self, code: u32, space: u32) {
        if space == 8 {
            self.result.push(code as u8);
        } else {
            let len = self.result.len();
            self.result[len - 1] |= code as u8;
        }
    }
}
